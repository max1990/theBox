...
def test_e2e_radar_false_then_true():
    db = DroneDB()
    em = EventManager(db)
    ...
    cue = {"object_id": "e2e-radar", "bearing_deg_true": 0.0, "bearing_error_deg": 5.0, "source_type": "radar"}
    sp.on_cue("object.sighting.directional", "payload", cue)
    time.sleep(1.2)

    assert captured.get("object_id") == "e2e-radar"
    # Radar sim writes heatmap on success
    assert sp.last_status.get("artifact_path", "").endswith("heatmap.png")
