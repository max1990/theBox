...
def cmd_mode(s: tcp_sender, mode_str: str):
    ...
    if mode_str not in modes:
        print("Invalid mode. Use RATE|CAGE|STOW|GEO"); sys.exit(2)
...
def cmd_jog(s: tcp_sender, az_rs: float, el_rs: float, ms: int):
    ...
    if az_rs: s.set_azimuth_rate(az_rs)
    if el_rs: s.set_elevation_rate(el_rs)
    time.sleep(max(ms, 0) / 1000.0)
    # stop motion
    if az_rs: s.set_azimuth_rate(0.0)
    if el_rs: s.set_elevation_rate(0.0)
    print("Jog complete.")
...
def cmd_sweep(s: tcp_sender, bearing_deg: float, spread_deg: float, steps: int, dwell_ms: int, el_deg: float):
    """
...
def main():
    ...
    sp = sub.add_parser("mode"); sp.add_argument("mode", choices=["RATE","CAGE","STOW","GEO"])
    sp = sub.add_parser("jog");  sp.add_argument("--az", type=float, default=0.0); sp.add_argument("--el", type=float, default=0.0); sp.add_argument("--ms", type=int, default=1000)
    sp = sub.add_parser("point"); sp.add_argument("--az-deg", type=float, required=True); sp.add_argument("--el-deg", type=float, required=True)
    ...
    sp = sub.add_parser("zoom"); sp.add_argument("--sensor", type=int, choices=[1,2,3], default=1); sp.add_argument("--cmd", choices=["RATE","HFOV","DZ_RATE","COMBINED"], required=True); sp.add_argument("--val", type=float, required=True)
    sp = sub.add_parser("status")
    sp = sub.add_parser("panic_recenter")

    args = p.parse_args()
    s = tcp_sender(); s.tcp_host=args.host; s.tcp_port=args.port

    if args.cmd == "mode": cmd_mode(s, args.mode)
    elif args.cmd == "jog": cmd_jog(s, args.az, args.el, args.ms)
    elif args.cmd == "point": cmd_point(s, args.az_deg, args.el_deg)
    elif args.cmd == "sweep": cmd_sweep(s, args.bearing_deg, args.spread_deg, args.steps, args.dwell_ms, args.el_deg)
    elif args.cmd == "zoom": cmd_zoom(s, args.sensor, args.cmd, args.val)
    elif args.cmd == "status": cmd_status(s)
    elif args.cmd == "panic_recenter": panic_recenter(s)
