...
class EventManager:
    ...
    def publish(self, event_type, data, publisher_name="system", store_in_db=True):
            ...
            for sub in self.subscriptions[event_type]:
                ...
                for path, value in data.items():
                    ...
                    if path.startswith(sub['field']):
                        ...
                        future = self.executor.submit(sub['callback'], event_type, path, value)
                        try:
                            terminate = future.result(timeout=5)
                            if terminate:
                                terminated_paths.add(path)
                        except TimeoutError:
                            print(f"Plugin {subscriber_name} timed out on event {event_type} for path {path}")
                            terminated_paths.add(path)
        ...
        all_notified = sorted(list(set(p for path_plugins in notified_plugins_per_path.values() for p in path_plugins)))
        was_any_terminated = bool(terminated_paths)
        
        self.log_event(event_type, data, publisher_name, all_notified, was_any_terminated)
