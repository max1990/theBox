...
class LearningPolicyShadow:
    ...
    def recommend(
        self, context: dict[str, Any], tiles: list[Tile]
    ) -> dict[str, Any] | None:
        return {
...
class MockVisionAdapter:
    ...
    def dispatch(
        ...
        tile: Tile,
        params: dict[str, Any],
        simulate_analyzer: Callable[[], dict[str, Any]],
    ):
...
class SearchPlannerPlugin(PluginInterface):
    ...
    def load(self):
        self.event_manager.subscribe(
            "object.sighting.directional", "payload", self.on_cue, 10
        )
    ...
    def _run_task(self, cue: dict[str, Any]):
        ...
        def budget_left() -> bool:
            ...
            if elapsed_ms > self.cfg.budgets.time_budget_ms:
                return False
            if (
                int(self.last_status.get("executed_tiles", 0))
                >= self.cfg.budgets.max_tiles
            ):
                return False
            return True
        ...
        try:
            ...
            bearing = float(
                cue.get("bearing_deg_true") or cue.get("bearing_deg") or 0.0
            )
            ...
            for tile in tiles:
                ...
                def simulate_analyzer() -> dict[str, Any]:
                    ...
                    if adapter is self.vision:
                        ...
                        if is_true:
                            artifact_path = (
                                f"{self.cfg.artifacts.artifact_dir}/artifact.jpg"
                            )
                    else:
                        is_true = idx >= 3
                        if is_true:
                            artifact_path = (
                                f"{self.cfg.artifacts.artifact_dir}/heatmap.png"
                            )
                    return {
                        "is_true": is_true,
                        "score": 0.9 if is_true else 0.2,
                ...
                try:
                    ...
                    per_tile_sla = (
                        self.cfg.timing.settle_ms
                        + self.cfg.timing.dwell_ms
                        + self.cfg.timing.analyzer_sla_ms
                    )
                    if elapsed_ms > per_tile_sla:
                        raise TimeoutError("analyzer SLA exceeded")
                except Exception:
                    self.last_status["timeouts"] = (
                        int(self.last_status.get("timeouts", 0)) + 1
                    )
                    if not budget_left():
                        self._set_state("FAILED")
                        return
                ...
                self.last_status["executed_tiles"] = (
                    int(self.last_status.get("executed_tiles", 0)) + 1
                )
