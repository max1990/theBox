...
class DroneShieldListenerPlugin(PluginInterface):
    ...
    def get_blueprint(self):
            ...
            if new_port:
                try:
                    ...
                    return jsonify(
                        {
                            "status": "ok",
                            "message": f"Port updated to {self.port}. Listener restarted.",
                        }
                    )
                except ValueError:
                    return (
                        jsonify({"status": "error", "message": "Invalid port number."}),
                        400,
                    )
        ...
        def create_fake_event():
            ...
            return jsonify(
                {"status": "ok", "message": "Fake event published successfully."}
            )
