...
class tcp_sender:
    ...
    exit_threshold_deg = 0.1  # how close before we say stop (0.1 degree)
    ...
    def enum_bytes(self, data, name=""):
        ...
        for i, byte in enumerate(data):
            val = (
                ord(byte) if isinstance(byte, str) else byte
    ...
    def read_data_packet(self, mclass, code, size, h_address, verbose=0):
        ...
        base_packet = struct.pack(
            "<BBBBHH", self.sync1, self.sync2, mclass, code, size, d_address
        )

        # Calc and append checksum bytes
        cs_dict = self.calc_checksum(base_packet)
        self.packet = (
            base_packet + self.packU8(cs_dict["sumA"]) + self.packU8(cs_dict["sumB"])
        )

        # for debugging
        if verbose:
            self.enum_bytes(self.packet, f"reading {h_address}")
        ...
        sync1, sync2, class_, code, size, address, payload, checksumA, checksumB = (
            fields
        )
        ...
        if (
            address == d_address
            and cs_dict["sumA"] == checksumA
            and cs_dict["sumB"] == checksumB
        ):
            if verbose:
    ...
    def seek_az_el(self, seek_az, seek_el):
        ...
        if (
            abs(az_diff) < self.exit_threshold_deg
            and abs(el_diff) < self.exit_threshold_deg
        ):
    ...
    def set_gimbal_mode(self, mode):
        ...
        # For Debugging
        self.enum_bytes(self.packet, f"Set Gimbal Mode : {mode}")
    ...
    def set_zoom_rate(self, sensor, command_word, value):
        ...
        base_packet = struct.pack(
            "<BBBBHBBf", self.sync1, self.sync2, 129, 2, 6, sensor, command, value
        )
        ...
        # For Debugging
        self.enum_bytes(
            self.packet, f"Set_Zoom_rate sent : {command_word} : {value}"
        )

        self.send_packet(self.packet, 0)
        print(f"Set_Zoom_rate sent : {command_word} : {value}")
    ...
    def setpoint_ecef(self, point_x, point_y, point_z):
        ...
        #                                     B          B          B B   H  d        d        d
        base_packet = struct.pack(
            "<BBBBHddd", self.sync1, self.sync2, 4, 129, 24, point_x, point_y, point_z
        )
        ...
        # For Debugging
        self.enum_bytes(
            self.packet, f"SetPoint ECEF to {point_x}|{point_y}|{point_z}"
        )
    ...
    def modify_mti(self, command, value):
        ...
        #                                      B          B          B   B H B       B      h           h
        base_packet = struct.pack(
            "<BBBBHBh", self.sync1, self.sync2, 130, 3, 3, command, value
        )
    ...
    def set_cage_az(self, az_rad: float) -> None:
        ...
        """
        base_packet = struct.pack(
            "<BBBBHf", self.sync1, self.sync2, 0x02, 0x82, 0x0004, az_rad
        )
    ...
    def set_cage_el(self, el_rad: float) -> None:
        ...
        """
        base_packet = struct.pack(
            "<BBBBHf", self.sync1, self.sync2, 0x02, 0x83, 0x0004, el_rad
        )
        cs = self.calc_checksum(base_packet)
        self.packet = base_packet + self.packU8(cs["sumA"]) + self.packU8(cs["sumB"])
