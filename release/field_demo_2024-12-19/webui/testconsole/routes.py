...
def register_routes(bp, event_manager, plugin_manager):
    ...
    def _do_slew(bearing):
        ...
        try:
            ...
            cam_connected = str(os.getenv("CAMERA_CONNECTED", "false")).lower() in {
                "1",
                "true",
                "yes",
                "on",
    ...
    def _do_vision(track_id, bearing):
        verified = str(os.getenv("VISION_VERDICT_DEFAULT", "true")).lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _do_cls(track_id, brand_model, affiliation, send_live=False):
        ...
        sentence = _wrap_sentence(
            "CLS",
            [track_id, "RF", "", brand_model, affiliation],
            f"details_url=http://127.0.0.1/drone/{track_id}",
        )
        dry_run = str(os.getenv("TEST_DRY_RUN", "true")).lower() in {
            "1",
            "true",
            "yes",
            "on",
    ...
    def _do_sgt(
        track_id, bearing, dist_m, err_m, brg_err, alt_m, alt_err, send_live=False
    ):
        from plugins.seacross_sender.plugin import _fmt_date_time, _wrap_sentence
    ...
    def test_console_index():
        ...
        dry_run = str(
            env_dict.get("TEST_DRY_RUN", os.getenv("TEST_DRY_RUN", "true"))
        ).lower() in {"1", "true", "yes", "on"}
        return render_template(
            "testconsole/testconsole.html", dry_run=dry_run, env_defaults=env_dict
        )
    ...
    def simulate_droneshield():
        _require_auth()
        bearing = float(
            request.form.get("bearing_deg", os.getenv("TEST_DEFAULT_BEARING_DEG", 45.0))
        )
        rssi = float(
            request.form.get("rssi_dbm", os.getenv("TEST_DEFAULT_RSSI_DBM", -72))
        )
        name = request.form.get("name", os.getenv("TEST_DEFAULT_NAME", "DJI AUT XIA"))
        protocol = request.form.get(
            "protocol", os.getenv("TEST_DEFAULT_PROTOCOL", "FHSS")
        )
        try:
    ...
    def step_seacross_sgt():
        ...
        send_live = request.form.get("send_live") == "on"
        out = _do_sgt(
            obj_id, bearing, dist_m, err_m, brg_err, alt_m, alt_err, send_live
        )
        return jsonify(out)
