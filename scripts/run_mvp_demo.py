...
from mvp.env_loader import get_float, get_str, get_bool
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("mvp_demo.log"),
...
class MockDBAdapter:
    ...
    def insert_detection(self, track_id: str, detection_data: dict, confidence: float, raw_data: str):
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "validated": sum(1 for t in self.tracks.values() if t.get("validated", False))
        }
...
class DroneShieldReplay:
    ...
    def load_detections(self):
        ...
        try:
            ...
            log.warning(f"Input file {self.input_file} not found - creating mock detections")
...
async def main():
    ...
    droneshield_input_file = get_str("DRONESHIELD_INPUT_FILE", "./data/DroneShield_Detections.txt")
    ...
    def on_detection(detection):
        ...
        track_id = db.upsert_track(detection["sensor_track_key"], detection["timestamp_ms"])
        db.touch_track(track_id, detection["timestamp_ms"])
        
        # Apply bearing offsets
        raw_bearing = detection["bearing_deg"]
        adjusted_bearing = apply_offsets(raw_bearing, 0.0, 0.0)  # No plugin offset, bow zero from env
        ...
        # Record detection
        db.insert_detection(track_id, detection, default_confidence, json.dumps(detection))
        ...
        async def do_vision_and_search():
            ...
            vision_result = await vision_plugin.run_verification(track_id, adjusted_bearing, now_ms)
            stats["vision_runs"] += 1
    ...
    # Replay detections
    replay = DroneShieldReplay(droneshield_input_file, droneshield_port, replay_interval_ms)
