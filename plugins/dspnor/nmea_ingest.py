...
from .constants import DEFAULT_NMEA_UDP_PORT, NMEA_RMC, NMEA_VTG, NMEA_GGA, NMEA_HDG
...
class NMEAParser:
    ...
    def parse_sentence(self, sentence: str) -> Optional[NMEAData]:
        """Parse NMEA sentence"""
        try:
            # Validate checksum
            ...
            self.logger.error("Failed to parse NMEA sentence", 
                            sentence=sentence[:50], error=str(e))
            return None
    ...
    def _parse_rmc(self, parts: list) -> Optional[NMEAData]:
        """Parse RMC sentence"""
        try:
            if len(parts) < 12:
                return None
    ...
    def _parse_vtg(self, parts: list) -> Optional[NMEAData]:
        """Parse VTG sentence"""
        try:
            if len(parts) < 10:
                return None
    ...
    def _parse_gga(self, parts: list) -> Optional[NMEAData]:
        """Parse GGA sentence"""
        try:
            if len(parts) < 15:
                return None
    ...
    def _parse_hdg(self, parts: list) -> Optional[NMEAData]:
        """Parse HDG sentence"""
        try:
            if len(parts) < 4:
                return None
    ...
    def _parse_timestamp(self, time_str: Optional[str], date_str: Optional[str] = None) -> datetime:
    ...
    def _parse_latitude(self, lat_str: Optional[str], lat_dir: Optional[str]) -> Optional[float]:
    ...
    def _parse_longitude(self, lon_str: Optional[str], lon_dir: Optional[str]) -> Optional[float]:
...
class NMEAUDPClient:
    ...
    def __init__(self, port: int = DEFAULT_NMEA_UDP_PORT, 
                 callback: Optional[Callable[[NMEAData], None]] = None):
        self.port = port
        self.callback = callback
        self.socket = None
        self.running = False
        self.parser = NMEAParser()
        self.last_data: Optional[NMEAData] = None
    ...
    def start(self):
        ...
        try:
            ...
            self.logger.error("Failed to start NMEA UDP client", port=self.port, error=str(e))
            self.stop()
    ...
    def get_current_velocity(self) -> Optional[tuple]:
        ...
        if (self.last_data.speed_over_ground is not None and 
            self.last_data.course_over_ground is not None):
