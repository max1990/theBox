...
from .constants import MPS_TO_KTS, KTS_TO_MPS, M_TO_KM, KM_TO_M
from .schemas import CAT010Track, NormalizedDetection, NMEAData
...
class DetectionNormalizer:
    ...
    def __init__(self, conf_map: str = "snr_db:linear:0:30", 
                 min_conf: float = 0.05, max_conf: float = 0.99,
                 range_units: str = "km", speed_units: str = "mps",
    ...
    def normalize(self, track: CAT010Track, current_heading: Optional[float] = None,
                 nmea_data: Optional[NMEAData] = None) -> Optional[NormalizedDetection]:
        ...
        try:
            ...
            if bearing is None:
                self.logger.warning("No position data available for track", track_id=object_id)
                return None
    ...
    def _extract_position(self, track: CAT010Track, 
                         current_heading: Optional[float]) -> Tuple[Optional[float], Optional[float]]:
    ...
    def _extract_velocity(self, track: CAT010Track) -> Tuple[Optional[float], Optional[float]]:
    ...
    def _apply_conf_mapping(self, track: CAT010Track) -> Optional[float]:
    ...
    def _calculate_distance_error(self, track: CAT010Track, distance_m: Optional[float]) -> Optional[float]:
    ...
    def _get_timestamp(self, track: CAT010Track) -> datetime:
        ...
        if track.time_of_day is not None:
            ...
            return now.replace(hour=hours, minute=minutes, second=int(seconds),
    ...
    def _create_raw_data(self, track: CAT010Track, 
                        nmea_data: Optional[NMEAData]) -> Dict[str, Any]:
    ...
    def _parse_conf_map(self, conf_map: str) -> Optional[Tuple[str, str, float, float]]:
