...
from .io_serial import MARASerialReader, SERIAL_AVAILABLE
...
class MARAPlugin(PluginInterface):
    ...
    def __init__(self, event_manager):
        ...
        self.rebroadcast_enable = os.getenv("OUT_REBROADCAST_ENABLE", "false").lower() == "true"
    ...
    def load(self):
        ...
        self.logger.info("Loading MARA plugin", 
    ...
    async def _start_serial(self):
        ...
        if not SERIAL_AVAILABLE:
            self.logger.error("Serial communication not available - install pyserial-asyncio")
            return
        ...
        while self._running:
            try:
                ...
                heartbeat = NormalizedDetection(
                    ...
                    raw={"heartbeat": True, "timestamp": datetime.now(timezone.utc).isoformat()}
                )
    ...
    def get_blueprint(self):
        ...
        def status():
            return jsonify({
                ...
                "udp_connected": self.udp_receiver.is_running if self.udp_receiver else False,
                "tcp_connected": self.tcp_client.is_connected if self.tcp_client else False,
                "serial_connected": self.serial_reader.is_connected if self.serial_reader else False
            })
