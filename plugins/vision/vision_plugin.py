...
class VisionPlugin:
    ...
    def _load_onnx_model(self):
        ...
        try:
            ...
            if Path(self.model_path).exists():
                ...
                log.warning("ONNX model file not found at %s - using defaults", self.model_path)
        except ImportError:
            log.warning("ONNX Runtime not available - using default verdict")
        except Exception as e:
            log.error("Failed to load ONNX model: %s - using defaults", e)

    async def run_verification(self, track_id: Union[str, int], bearing_deg: float, now_ms: int) -> VisionResult:
        """
    ...
    async def _run_detection(self, frame: np.ndarray, roi_bounds: Tuple[float, float, float, float]) -> List[Detection]:
    ...
    def _parse_yolo_outputs(self, outputs: np.ndarray, roi_bounds: Tuple[float, float, float, float]) -> List[Detection]:
    ...
    def _check_verification(self, track_id: str) -> Optional[Track]:
